...
def simular_ruptura(wn, start_time, pipe_name):
    ...
    for time in sim_pressure.index:
        for node in sim_pressure.columns:
            ...
            all_simulation_data.append((tempo_str, node, pipe_name, pressure, demand, quebrado_ou_vazamento, pipe_impact, x_coord, y_coord))
        
        # Armazenar dados para a própria tubulação quebrada (sem coordenadas)
        all_simulation_data.append((tempo_str, pipe_name, pipe_name, 0, avg_flowrate, 0, 1, None, None))

# Loop para definir o número de simulações e parâmetros aleatórios
num_simulacoes = 1  # Número de simulações desejado
for sim in range(num_simulacoes):
    ...
    start_time = random.randint(1, 2) * 3600  # Tempo aleatório de início da ruptura (entre 1 e 3 horas)
...
cursor.executemany('''
    INSERT INTO rupturas (tempo, no, tubulacao, pressao, vazao, quebrado_ou_vazamento, submetido_a_quebra, x_coord, y_coord)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
''', all_simulation_data)
